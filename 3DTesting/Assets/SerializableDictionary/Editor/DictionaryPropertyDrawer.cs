    ...
    public class DictionaryPropertyDrawer : PropertyDrawer
    {
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            if (property.isExpanded)
            {
                ...
                {
                    index += keysValues.arraySize - 1;
                }

                //Normal height for this array, it's + 3 because it's normal property height, a line for the main reorder and add button and empty space line
                height += (index * 20) + 1;

                for (int t = 0; t < keysValues.arraySize; t++)
                {
                    int keyIndex = t;
        ...
        private void List_drawHeaderCallback(Rect rect, GUIContent label)
        {
            ...
            list.List.isExpanded = property.isExpanded = EditorGUI.Foldout(rect, property.isExpanded, title, true);
        }
        ...
        private void List_drawElementCallback(Rect rect, SerializedProperty element, GUIContent label, int index, bool selected, bool focused)
        {
            ...
            if (valueProp.propertyType == SerializedPropertyType.Generic)
            {
                ...
                if (keyValueProp.type.Equals("Quaternion"))
                {
                ...
                if (keyValueProp.isExpanded)
                {
                    ...
                    if (valueProp.type.Equals("Quaternion"))
                    {
                        EditorGUI.BeginChangeCheck();
        ...
        private Rect GetNextRect(ref Rect position)
        {
            ...
            var h = lineHeight + 1f;
            position = new Rect(position.x, position.y + h, position.width, position.height = h);
            return r;
        }
